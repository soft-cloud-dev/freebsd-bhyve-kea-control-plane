...
func (f *fakeRepository) InspectResource(context.Context, string) (state.Inspection, error) { return f.inspection, nil }
func (f *fakeRepository) StartOperation(context.Context, string) error { return nil }
func (f *fakeRepository) ExecutionSteps(context.Context, string) ([]state.OperationStep, error) { return f.steps, nil }
func (f *fakeRepository) ClaimStep(_ context.Context, _ string, sequence int) (state.OperationStep, error) {
	...
	for _, step := range f.steps {
		if step.Sequence == sequence { return step, nil }
	}
	panic("missing step")
}
...
func (f *fakeRepository) FailStep(context.Context, string, int, string, string, bool) error { return nil }
func (f *fakeRepository) CompleteOperation(context.Context, string, string, string) error { f.finished = true; return nil }
...
func TestExecutorResumesAtFirstIncompleteStep(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(driver.calls) != 1 || driver.calls[0] != 2 { t.Fatalf("driver calls: %#v", driver.calls) }
	if len(repo.claimed) != 1 || repo.claimed[0] != 2 || !repo.finished { t.Fatalf("repo state: %#v", repo) }
}
...
func TestExecutorBlocksTamperedInput(t *testing.T) {
	...
	if err == nil { t.Fatal("expected tampered input to fail") }
}
